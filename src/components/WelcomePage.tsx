...
// ======================================================
// 🧠 PERSISTENCE KEY — remembers that the one automatic opening
// has already happened, even after a logout or page reload.
// ======================================================

const AUTO_OPEN_USED_KEY = 'birthday-auto-open-used'
...
const balloons: Balloon[] = [
  { id: 1, image: img6, left: '7%',  top: '18%', color: '#10ce1a', rotate: -8 },
  { id: 2, image: img1, left: '18%', top: '78%', color: '#19c8e3', rotate: 7 },
  { id: 3, image: img2, left: '5%',  top: '52%', color: '#f9f918', rotate: -5 },
  { id: 4, image: img5, left: '93%', top: '18%', color: '#f973a5', rotate: 7 },
  { id: 5, image: img3, left: '82%', top: '78%', color: '#d82dcd', rotate: -7 },
  { id: 6, image: img4, left: '95%', top: '52%', color: '#a78bfa', rotate: 6 },
]
...
export function WelcomePage({ countdown, onOpen }: WelcomePageProps) {
  ...
  const openAutomaticallyOnce = () => {
    // Reaching zero opens the surprise without a PIN only once.
    // Persist the flag before opening so it cannot auto-open again after logout.
    if (localStorage.getItem(AUTO_OPEN_USED_KEY) === 'true') return

    localStorage.setItem(AUTO_OPEN_USED_KEY, 'true')
    openSurprise()
  }
  ...
  return (
    <section className="relative mx-auto flex min-h-screen max-w-6xl flex-col items-center justify-center overflow-hidden px-5 py-12 text-center">
      <div className="welcome-balloons" aria-hidden={isPinOpen}>
          ...
          return (
            <button
              ...
              className={`welcome-balloon ${
                isPopping ? 'welcome-balloon-popping' : ''
              }`}
              style={
                {
                  left: balloon.left,
      ...
      <div className="relative z-10">
        ...
        <div className="mt-7 rounded-[2rem] border border-[#f3d6df] bg-white/65 px-5 py-5 shadow-[0_14px_40px_rgba(168,77,111,.10)] sm:px-8">
          <Countdown values={countdown} onComplete={openAutomaticallyOnce} />
        </div>
